...
begin
    -- 레코드 변수에 값을 저장
     ...
     v_dept_row.loc := 'Seoul Gangnam';
     
     -- 레코드 변수의 값을 읽어올 때: 레코드 변수.필드
...
declare
   ...
   -- 테이블의 일부 컬럼만 검색하거나 두 개 이상의 테이블에서 조인하는 경우 
...
-- 변수들을 저장 할 수 있는 레코드(emp_dept)를 정의하세요.
-- 조인(join) 문장을 작성해서 SCOTT의 정보를 출력하는 PL/SQL을 작성하세요
...
-- emp 테이블, salgrade 테이블에서 
-- 사번, 이름, 급여, 급여 등급
-- 4개의 변수들을 저장 할 수 있는 레코드(emp_sal)를 정의하세요.
-- 조인(join) 문장을 작성해서 SCOTT의 정보를 출력하는 PL/SQL을 작성하세요
