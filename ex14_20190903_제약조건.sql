...
--default: 컬럼의 기본값(insert하지 않아도 자동으로 저장되는 값)을 설정 -엄밀하게 말하면 오라클에서 지칭하는 제약조건은 아님
...
--테이블 이름: ex06
--컬럼: col - varchar2(20), 제약 조건은 삽입되는 문자열의 길이가 8이상
create table ex06 (
...
insert into ex_emp_1
values (1001, '오쌤', 10);

--ex_emp_1에는 ex_dept 테이블에 생성되어 있지 않은 부서 번호는 insert할 수 없다!
--FK가 설정된  ex_emp_1 테이블의 deptno 컬럼에는 null은 가능
insert into ex_emp_1 (empno, ename)
values (1001,'오쌤');

insert into ex_dept values (10, '개발팀');
insert into ex_dept values (20, '분석팀');

insert into ex_emp_1 values(2001, '홍길동', 10);
insert into ex_emp_1 values(3001, 'scott', 20);

select*from ex_emp_1;
select*from ex_dept;
commit;
