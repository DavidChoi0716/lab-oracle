...
for-loop에서 사용하는 변수는 declare 구문에서 선언하지 않음!
...
-- 2) continue when 조건식: 조건식을 만족하는 경우에 반복문의 시작 부분으로 돌아감 
