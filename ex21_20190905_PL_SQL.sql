...
-- SQL에 대한 절차적 언어 확장 가능


-- PL/SQL의 필요성
-- 직원(emp) 테이블에서 부서별 직원들의 정보를 출력
select * from emp where deptno = ?;
-- 어떤 부서 번호들이 있는 지를 확인하기 위해서 부서(dept) 테이블을 확인
...
--하나의 업무를 완수하기 위해서 작업을 절차적으로 진행해야 할 필요가 잇다.
...
o. declare, begin, exception 에는 세미콜론(;)을 사용하지 않음.
o. end와 각 문장들(변수 선언 문장, 실행문, 예외 처리문, ...) 끝에는 
세미콜론(;)을 반드시 사용해야 함.
o.PL/SQL 블록 내부에도 주석들을 사용할 수 있음.
...
1) 변수를 선언하면서 동시에 값을 할당(초기화)_assignment_initialize 하는 경우
   변수이름 데이터타입 := 값;
...
--intialize, initialization (초기화): 변수에 최초로 값을 저장(할당)하는 것
...
accept p_가로길이 prompt '직사각형의 가로 길이는 무엇~?';
accept p_세로길이 prompt '직사각형의 세로 길이는 무엇~?';

declare
  v_직사각형의넓이 number := &p_가로길이*&p_세로길이;
  
begin
   dbms_output.put_line('직사각형의 넓이는' || v_직사각형의넓이);
end;
/
...
begin
  dbms_output.put_line('넓이:' || (&p_width*&p_height));
