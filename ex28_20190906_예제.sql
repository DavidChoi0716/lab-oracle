...
-- x <= r < y
select dbms_random.value(0, 1) from dual; -- 0 <= r < 0.999999 .......
select dbms_random.value(0, 2) from dual; -- 0 <= r < 1.999999 .......
...
declare
    v_trials constant number := 100000; --실험(반복) 횟수
    v_coin number; --동전의 앞/뒷면( 0 또는 1 )을 저장할 변수
    --동전이 앞면일 때( v_coin = 0) 카운트를 1증가시킬 변수
    v_head number := 0; 
    --동전이 뒷면일 때( v_coin = 1) 카운트를 1증가시킬 변수 
    v_tail number := 0; 
...
declare 
  ...
  -- 원점에서의 거리가 1미만인 경우 1씩 증가시킬 변수 
  hit number := 0;
  trials constant number := 100000;
  pi number; 
