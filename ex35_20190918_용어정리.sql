...
2. 저장 서브 프로그램(stored sub program): 이름이 있는 PL/SQL 블록
    ...
    - 오라클 서버에 저장되는 객체(object) > 재사용(re-use)이 가능 
...
-- in out: 값을 전달 받기도 하고, 값을 반환하기도 하는 매개변수
...
create or replace procedure my_proc3(
  ...
  out_deptno out emp.deptno%type -- out 모드 파라미터
)
...
--    (2) p_total - out 모드, 숫자타입
--    (3) p_tax - in 모드, 숫자타입, 기본값 = 0.05
--    
...
exception 
    when others then
        dbms_output.put_line(p_empno || '사번 정보는 없습니다.');
         p_total := 0;
